...
import { PostDataToken } from "../../pages/Auth/service";
...
const ProductsCard: React.FC<ProductsCardProps> = ({ item, handleOpen }) => {
    ...
    const postLike = (id, status) => {
        const data = {
            is_liked: status,
        };
        PostDataToken(`product/${id}/like/`, data).then(() =>
            console.log("ok")
        );
    };
                    ...
                    <div className="absolute z-[999] top-1 right-3 flex gap-2">
                        {item?.is_liked ? (
                            <IoMdHeart
                                size={20}
                                color="red"
                                onClick={() => postLike(item.id, false)}
                            />
                        ) : (
                            <IoMdHeartEmpty
                                size={20}
                        ...
                        <div className="hidden group-hover:block">
                            ...
                            {item?.warehouse?.length > 0 &&
                                productItem?.warehouse?.map((item, i) => (
                                    <p
                                        key={i}
                                        className="text-fs_8 opacity-70 font-medium "
                                    >
