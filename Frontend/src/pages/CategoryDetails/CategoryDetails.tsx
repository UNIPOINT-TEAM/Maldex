...
const CategoryDetails = () => {
  ...
  useEffect(() => {
    if (response?.discounts) {
      const filterDiscount = response.discounts.sort((a, b) => a.name - b.name);
            ...
            >
              {CategoryTabs.map(({ label, value }) => (
                <Tab
                  ...
                  className="text-[9px]  p-0  font-Helvetica-Neue uppercase h-[25px] text-darkSecondary w-auto font-bold text-start "
                >
                  <p
                    className={`${
                      activeTab === value
              ...
              <div className="flex flex-col gap-2 bg-white px-3 py-5 rounded-s-xl">
                {response?.colors?.map((item) => (
                  <input
                    ...
                    type="radio"
                    name="color"
                    style={{ accentColor: item.hex, background: item.hex }}
                    className={`${
